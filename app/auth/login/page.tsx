    ...
    <div className="min-h-dvh flex flex-col items-center justify-center px-5">
      ...
      {sent ? (
        <div className="text-center flex flex-col gap-3">
          <div className="font-vt323 text-[20px] text-slop-ink uppercase tracking-wide">check your email</div>
          <div className="font-courier text-[14px] text-slop-secondary">
            tap the link sent to <span className="text-slop-ink">{email}</span>
          </div>
          <button
            onClick={() => setSent(false)}
            className="font-vt323 text-[14px] text-slop-dim uppercase tracking-wide mt-4"
          >
            ← use different email
          </button>
        </div>
